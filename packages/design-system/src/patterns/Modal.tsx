    ...
    >
      {(title || showClose) && (
        <div className="modal-h" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
          ...
          {showClose ? (
            <button type="button" className="x" aria-label="Close dialog" onClick={() => onOpenChange(false)} style={CLOSE_BTN_STYLE}>
              ×
            </button>
          ) : null}
        </div>
    ...
    >
      {(title || showClose) && (
        <div className="drawer-h">
          ...
          {showClose ? (
            <button type="button" className="x" aria-label="Close panel" onClick={() => onOpenChange(false)}>
              ×
            </button>
          ) : null}
        </div>
